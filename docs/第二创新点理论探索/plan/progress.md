...
日期：2026-08-23 至 2026-08-24。

当前阶段：L1正式选择已完成并判定STOP；MC-CVLRA归档，不进入确认，第二创新点仍未成立。
...
## L1正式选择与止损回填（2026-08-24）

- 按冻结预算运行24→1和168→6、种子2060–2062、八个变体；正式test六项访问计数均为0。
- 独立复算完整性为PASS，正式阶段门为STOP。
- 168→6中F11相对F10为3/3改善，但平均仅0.477392%，低于0.5%门。
- F11相对B1/B2平均改善仅0.073843%/0.022306%，均低于0.2%；相对B-flat为0/3且平均−0.161018%。
- M-shuffle种子2061的增益损失仅19.1907%，未达到每种子50%门。
- 逐horizon、高污染安全和24→1非劣门虽通过，但不得替代失败的实际收益与结构特异性门。
- 唯一决定：归档MC-CVLRA，不创建confirmation分支，不打开正式test，不运行北京/广州确认，不扫描配置挽救。
- 详见[08_L1正式选择止损报告](../08_L1正式选择止损报告.md)和`experiments/results/cross_variable_lag_adapter/l1_selection_summary/`。
