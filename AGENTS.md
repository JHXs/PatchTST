...
文档中的编码器后空间注意力与 VMD 在 main 未实现。频域/多尺度族已经执行过：`experiment/causal-multiscale-frequency@969be9a` 以预测端残差形式实现固定 rFFT 与因果局部三尺度滤波器组，两者均未通过等容量时域特异性门（rFFT 相对锁定 ST 平均改善 0.3502%，但落后等容量时域控制 0.4024%；滤波器组平均改善 0.2666%，平均 RMSE 50.1954 劣于等容量控制 49.9519，六个预测步均未胜出），据此止损，不得以换名方式复活。
...
可以表述为“最终固定结构在北京1013站PM2.5的24→1和168→6任务中稳定优于退化PatchTST（以RMSE为主指标）”。内部严格放行状态为true：两任务均满足5/5方向一致、单侧精确符号检验p=0.03125和预注册实际收益门。北京1013的24→1 SMAPE仍上升，不能声称该站所有指标均有提升。

跨城市泛化已完成（round14，用户2026-09-11授权的PIVOT-B）：在跨城市目标域广州的8个未消费中心站（b1）、5个新种子（7001–7005）与从未用于结构选择的确认时间段上，同一固定结构相对同种子退化PatchTST的池平均配对RMSE降幅为24→1 **2.2072%**、168→6 **1.8224%**，8/8站与5/5块方向一致，两任务SMAPE同时改善；入口为`experiment/cross-city-generalization-ablation@5211d7b`的`docs/round14-跨城市泛化确认/13_跨城市泛化确认结论.md`。

该泛化的声明边界：不是第二创新机制（本轮未新增机制，只是第一创新的外部有效性验证）；不得称为全新城市或完全盲测（广州9022、9027已被历史消费，本轮通过排除与封存保证独立性）；不得外推到其他城市、其他污染物或其他时间段；广州b2/b3保留池未被使用。

频域/多尺度族已由 `experiment/causal-multiscale-frequency@969be9a` 执行并止损，不再是待启动方向；任何写入预测端加性残差槽的新机制，必须同时胜过同信息集基线与“容量匹配但去除所声称结构”的对照臂（NC13，详见 `docs/研究探索与实验验证工作流.md` §4.4）。
...
- **main 上的任何改动都必须先获批。** 包括代码、文档、索引、登记表、图表与结果产物。代理先在自己的分支或临时工作区完成改动，向用户提交 diff 摘要与影响面，得到明确批准后才允许推送 main。
- **禁止代理自发向 main 提交或推送。** 分支内的探索、实验、失败记录与中间产物提交不受此限制，可自行进行。
- **每次请求推送 main 时，必须同时报告三项**：改了哪些文件；是否触碰模型/数据/协议；若需回退，回到哪个提交最干净。
- **"记账"类改动同样适用**：`docs/研究路线总表.md`、`docs/提交记录中文对照表.md`、`figures/data-manifest.md`、`tables/table-schema.md`、`plan/*` 等都不再直接改 main。
- 例外：用户在当前对话中明确说"提交/推送/合进 main"的那一项，仅对该项生效，不外推到其他改动。
